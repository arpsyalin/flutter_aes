...
class FlutterAes {
  static const MethodChannel _channel = MethodChannel('flutter_aes');
  ...
  static Future<String?> encryptString(String? txt, String? key,
      {String? iv}) async {
    try {
      final String? result = await _channel
          .invokeMethod('encrypt', {"input": txt, "key": key, "iv": iv});
      return result;
    } on PlatformException catch (e) {
      throw "Failed to get string encoded: '${e.message}'.";
    }
  ...
  /// [txt] and [key] shoudn't be null.
  static Future<String?> decryptString(String? txt, String? key,
      {String? iv}) async {
    try {
      final String? result = await _channel
          .invokeMethod('decrypt', {"input": txt, "key": key, "iv": iv});
      return result;
    } on PlatformException catch (e) {
      throw "Failed to get string encoded: '${e.message}'.";
    }
